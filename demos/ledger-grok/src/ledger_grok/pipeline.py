"""Transit-search pipeline for the SAPIENS x Grok "LEDGER" demo.

Deliberately simple, honest, documented methods (no fake sophistication):

1. load a CSV light curve (time in BKJD = BJD - 2454833, PDCSAP flux),
2. median-normalise and divide by a running-median baseline,
3. box-least-squares (BLS) grid search over period and duration
   (a simplified, unweighted Kovacs et al. 2002-style matched filter),
4. measure depth, epoch and SNR by phase-folding at the best period,
5. adversarial self-checks: harmonic powers, odd/even depth, secondary dip.

Pure Python standard library only; no third-party packages, no network.
"""
...
def load_csv(path: str | Path) -> tuple[list[float], list[float]]:
    """Load a two-column CSV (time, flux); '#' lines are comments."""
    times: list[float] = []
    fluxes: list[float] = []
    with Path(path).open(newline="", encoding="utf-8") as fh:
        for row in csv.reader(fh):
            if not row or row[0].lstrip().startswith("#"):
                continue
            if row[0].strip().lower() in ("time_bkjd", "time"):  # column header row
                continue
...
def detrend(times: list[float], fluxes: list[float], window_days: float = 1.5) -> list[float]:
    ...
    The window (~1.5 d) is far wider than the ~2 h transit, so the transit
    itself is not flattened away.
    """
    n = len(times)
    base = _median(fluxes)
...
def _eval_period(times: list[float], resid: list[float], period: float, ms: list[int]) -> dict:
    ...
    mmax = max(ms)
    ext = ys + ys[:mmax]  # wrap-around copy so boxes crossing phase 1.0 work
    pre = [0.0]
    acc = 0.0
    for v in ext:
        acc += v
    ...
    for m in ms:
        ...
        if best is None or power > best[0]:
            best = (power, s, m, i0)
    power, s, m, i0 = best
    # Residual-weighted centroid of the in-box phases: a much more stable
    # mid-transit estimate than box edges (box width >= true transit width).
    phi_start = ph[i0]
    wsum = 0.0
    csum = 0.0
...
def bls(
    ...
    """Box-least-squares grid search, uniform in frequency, plus a fine refine pass."""
    n = len(times)
    if n < 20:
...
def fold_measure(
    ...
    for i, ph in enumerate(phases):
        ...
        if d <= half:
            ins.append(y[i])
        elif d > half * 1.6:  # buffer zone excludes ingress/egress wings
            outs.append(y[i])
    ...
    t0 = min(times)
    # phase_center is an ABSOLUTE fold phase (fmod(t, period) origin at t=0),
    # so transit centers are (m + phase_center) * period for integer m; pick
    # the one nearest the window start.
...
def harmonic_powers(
    ...
    """Box power at P/2, P and 2P (transits coincide modulo 2P, so power at 2P
    staying high is expected; the real discriminators are odd/even + secondary)."""
    n = len(times)
    resid = [1.0 - v for v in y]
    ms = sorted({max(2, int(round(q * n))) for q in durations})
    out = {}
